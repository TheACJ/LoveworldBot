-- Loveworld Bot Database Schema for Supabase
...
CREATE TABLE IF NOT EXISTS scraping_jobs (
    ...
    user_id BIGINT NOT NULL REFERENCES bot_users(telegram_user_id),
...
-- Admin users can access all data (if admin_ids column exists)
CREATE POLICY "Admins can access all data" ON scraping_jobs
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM bot_users 
            WHERE telegram_user_id = current_setting('app.current_user_id')::BIGINT 
            AND is_admin = TRUE
        )
    );
...
-- Create storage bucket SQL (run this in Supabase Storage section)
